...
int main(int argc, char *argv[])
{
    ...
    printf("进程句柄=%p \r\n", 进程句柄);
    unsigned int 返回值 = 0;
    ReadProcessMemory(进程句柄, (LPCVOID)0x1841D2BA400 /*要读取的地址*/, &返回值 /*存放数据的地址*/, 4, 0);
    printf("返回值=%X  返回值=%d 返回值=%u\r\n", 返回值, 返回值, 返回值); ///  GetLastError();

    /// 要写入的新值
    unsigned int 新值 = 42; /// 替换为您要写入的值
    if (!WriteProcessMemory(进程句柄, (LPVOID)0x1841D2BA400, &新值, sizeof(新值), nullptr))
    {
